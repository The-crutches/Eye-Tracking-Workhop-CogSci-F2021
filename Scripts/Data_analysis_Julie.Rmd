...
Fixations <- read_csv("~/Uni stuff/Computational Modeling/Eye-Tracking-Workhop-CogSci-F2021/data/Fixations_final.csv", guess_max = Inf)
Saccades <- read_csv("~/Uni stuff/Computational Modeling/Eye-Tracking-Workhop-CogSci-F2021/data/Saccades_final.csv")
```
...
Saccades_search <- filter(Saccades, SearchOrCount == "Search")
Saccades_count <- filter(Saccades, SearchOrCount == "Count")

dens <- density(Saccades_search$Amplitude)
plot(dens)


dens_c <- density(Saccades_count$Amplitude)
...
df <- filter(Saccades, Task == "Visual foraging")
df$Task <- as.factor(df$Task)

mGaus <- lmer(Amplitude ~ 0 +SearchOrCount + (1|ParticipantID), data = df)
...
```{r}
df <- df %>% filter(Amplitude > 0)



df$Amplitude_log <- log(df$Amplitude)


mLog <- lmer(Amplitude_log ~ 0 +SearchOrCount + (1|ParticipantID), data = df)
...
fix_df <- filter(Fixations, Task == "Visual foraging")
fix_df$Task <- as.factor(fix_df$Task)

Fixation_search <- filter(fix_df, SearchOrCount == "Search")
Fixation_count <- filter(fix_df, SearchOrCount == "Count")

dens <- density(Fixation_search$Duration)
plot(dens)


dens_c <- density(Fixation_count$Duration)

plot(dens_c)
plot(dens)

ggplot(fix_df, aes(Duration, fill = SearchOrCount))+
...
```{r}
Fixations_soc <- Fixations %>% 
  filter(Task == "Social engagement")




Fixations_soc$Ostensiveness <- as.factor(Fixations_soc$Ostensiveness)

ggplot(Fixations_soc, aes(x = PupilSize, color = Ostensiveness))+
         geom_density()
...
mPupil <- glmer(PupilSize ~ 0 + Ostensiveness:Direction + (1|ParticipantID), data = Fixations_soc) 
...
```{r}
ggplot(Fixations_soc, aes(x = FixationNo, y = PupilSize, color = Ostensiveness))+
  geom_smooth(method = "gam")+ facet_wrap(~Direction)
...
```{r} 
mPupil2 <- glmer(PupilSize ~ 0 +FixationNo + Ostensiveness*Direction + (1+FixationNo|ParticipantID), data = Fixations_soc)
