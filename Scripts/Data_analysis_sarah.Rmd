...
Fixations <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/Fixations_final.csv", guess_max = Inf)
Saccades <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/Saccades_final.csv")

Fixations$z_scores_duration <- (Fixations$Duration - mean(Fixations$Duration)) / sd(Fixations$Duration)

Fixations <- Fixations %>% filter(z_scores_duration > -3 & z_scores_duration < 3 )
```
...
Saccades_search <- Saccades %>% filter(SearchOrCount == "Search")
Saccades_count <- Saccades %>% filter(SearchOrCount == "Count")


dens <- density(Saccades_search$Amplitude)
dens <- density(Saccades_count$Amplitude)
plot(dens)
...
Saccades <- Saccades %>% mutate( 
  ...
  Task = as.factor(Task),
  SearchOrCount = as.factor(SearchOrCount)
)

SaccadesVF <- Saccades %>% filter(Task ==)

mGaus <- lmer(Amplitude ~ Task + (|ParticipantID) , Saccades)
...
Fixations <- Fixations %>% filter(Task == "Social engagement")

Fixations %>% ggplot(aes(PupilSize, color = Direction)) + geom_density()

Fixations %>% ggplot(aes(PupilSize, color = Ostensiveness)) + geom_density() + geom_density(aes(color = Direction))
```
...
Fixations <- Fixations %>% mutate(across(where(is_character, (as_factor))))
Fixations <- Fixations %>% mutate(across(where(is_character),as_factor))

Fixations <- Fixations %>% mutate_if(is.character,as.factor)

mPupil <- glmer(
  PupilSize ~ 0 + FixationNo*Direction*Ostensiveness + (1 + FixationNo|ParticipantID) + (1|ParticipantID:Trial), 
                family = Gamma(
                link = "identity"),
                data = Fixations)
...
Fixations %>% ggplot(aes(FixationNo, PupilSize, color = Ostensiveness)) + geom_smooth(method = 'gam', alpha = 0.3) +
  geom_smooth(aes(color = Direction), alpha = 0.3)
...
Fixations %>% ggplot(aes(Duration, color = Ostensiveness)) + geom_density() + geom_density(aes(color = Direction))
...
mDuration <- glmer(Duration ~ 0 + FixationNo*Ostensiveness*Direction + (1|ParticipantID) + (1|ParticipantID:Trial),
...
Fixations$p <- predict(mDuration, data = Fixations)

Fixations %>% ggplot(aes(Duration)) + geom_point(aes(p))
