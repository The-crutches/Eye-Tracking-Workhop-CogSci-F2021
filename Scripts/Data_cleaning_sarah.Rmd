...
foraging_fixations <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/foraging_fixations.csv")
soceng_fixations <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/soceng_fixations.csv")

## Load saccades data for both experiments
foraging_saccades <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/foraging_saccades.csv" )
soceng_saccades <- read_csv("C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/soceng_saccades.csv" )
...
Fixations1 <- Fixations %>% select(Task) # looks right
...
```{r}
#print(colnames(Fixations))

#print(colnames(Saccades))
```
...
Fixations = Fixations %>% 
  select(
    ...
    EyeTracked= EYE_USED,
    Task,
    total_fix = TRIAL_FIXATION_TOTAL,
...
ggplot(Fixations, aes(x = PositionX)) +
  geom_bar()

ggplot(Fixations, aes(x = PositionY)) +
  geom_bar()

ggplot(Fixations, aes(x = PupilSize)) +
  geom_bar()
...
Fixations <- Fixations %>% mutate(
  PositionX = PositionX / 100,
  PositionY = PositionY / 100,
  PupilSize = PupilSize / 100,
  ParticipantID = as.factor(ParticipantID),
  Trial = as.factor(Trial),
  EyeTracked = as.factor(EyeTracked),
  Task = as.factor(Task)
)
...
Fixations <- Fixations %>% filter(
  PositionX <= 1680 & PositionX >= 0,
  PositionY <= 1050 & PositionY >= 0
)
  

Saccades <- Saccades %>% filter(
  StartX <= 1680 & StartX >= 0,
  EndX <= 1680 & EndX >= 0,
  StartY <= 1050 & StartY >= 0,
  EndY <= 1050 & EndY >= 0
)

Fixationssaved = Fixations
...
print(range(Fixations$PositionX))
print(range(Fixations$PositionY))

print(range(Saccades$StartX))
print(range(Saccades$StartY))
...
img <- jpeg::readJPEG('C:/Users/Sarah/Desktop/eyetracking/Eye-Tracking-Workhop-CogSci-F2021/data/stimuli_Foraging/space_capsules.jpg')  
...
Fixations <- Fixations %>% mutate(FixationNo = as.factor(FixationNo))

ggplot(Fixations, aes(x =  Duration)) +
  geom_boxplot(width = 0.5)

## Check for outliers in saccade amplitude using a boxplot

ggplot(Saccades, aes(x =  Amplitude)) +
  geom_boxplot(width = 0.5)

## Check for outliers in saccade mean velocity using a boxplot

ggplot(Saccades, aes(x =  MeanVelocity)) +
  geom_boxplot(width = 0.5)

## Check for outliers in saccade peak velocity using a boxplot

ggplot(Saccades, aes(x =  PeakVelocity)) +
  geom_boxplot(width = 0.5)
...
Fixations$z_scores_duration <- (Fixations$Duration - mean(Fixations$Duration)) / sd(Fixations$Duration)

Fixations1 <- Fixations %>% filter(z_scores_duration > -3 & z_scores_duration < 3 )

Fixations2 <- Fixations %>% filter(Duration < mean(Fixations$Duration) + (3* sd(Fixations$Duration)))
