...
Fixations <- Fixations %>% select(-PositionY) %>% rename(PositionY = PositionY_flipped)
...
make_scanpath <- function(participant, trial){
  ...
  p <- ggplot(x, aes(PositionX, PositionY_flipped, color = FixationNo)) + 
...
for (i in Fixations$ParticipantID) {
  make_scanpath("nanna1", i)
}
...
```{r}
make_heatmap2 <- function(participant, trial){
  
  ## Select only the participant/trial of interest
  x <- Fixations2[Fixations2$ParticipantID == participant & Fixations2$Trial == trial,]
...
Fixations <- Fixations %>% filter(Task == "Visual foraging")

make_heatmap2("nanna1", 8)
...
make_heatmap1 <- function(stimulus){
  
  ## Select only the participant/trial of interest
  x <- Fixations2[Fixations2$Stimulus == stimulus,]
